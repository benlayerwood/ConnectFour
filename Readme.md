...
Innerhalb der Test-Klasse können die 5 geforderten Tests mit der Methode runTests() aufgerufen werden. Die Testergebnisse werden auf der Konsole angezeigt.
...
Mit der Methode playAlphaBeta() kann man den Alpha-Beta gegen sich selbst spielen lassen und den Fortschritt auf der Konstole überprüfen, während man mit der Methode playAgainstAlphaBeta() selbst gegen den Alpha-Beta spielen kann.
...
### Alpha-Beta
#### Allgemeines
Die Methode alphaBeta wird von der Methode bestMove aufgerufen und liefert ein Pair<Int?,Int> zurück, bei welchem der erste Wert der bestmögliche Zug ist und der zweite Wert die dazugehörige Bewertung. Der erste Wert kann null werden, weil innerhalb vom Alpha-Beta-Algorithmus manchmal nur Bewertungen zurückgegeben werden, aber keine Züge. In diesem Fall wird der erste Pair-Wert null und der zweite Wert entspricht der Bewertung.
...
* maximize (Ist der aktuelle Spieler der maximierende Spieler?)
* alpha (Alpha-Wert des maximierenden Spielers)
* beta (Beta-Wert des minimierenden Spielers)
* cache (HashMap zum Zwischenspeichern von Spielstellungen)).
...
* Nun folgt die Abbruchbedingung des rekursiven Algorithmus: Wurde die Tiefe 0 erreicht oder ist das Spiel vorbei, können folgende Fälle zutreffen:
    ...
    * Die Tiefe wurde erreicht und noch kein Spieler hat gewonnen -> Bewertung = averageBoard()
...
* Danach folgt der "Kern" des Alpha-Beta-Algorithmus
    ...
    * Fall 2: Minimierender Spieler ist am Zug
        * (Schleifenbeginn) Es werden alle möglichen Züge in zufälliger Reihenfolge durchlaufen
        * <b>Erweiterung</b>: Ist es bereits ersichtlich, dass der nächste Zug für den minimierenden Spieler einen Gewinn bringt, wird dieser Zug und die min. Bewertung zurückgegeben
        * Alpha-Beta wird nach dem Spielen dieses Zuges rekursiv aufgerufen
...
Die Oberfläche wurde in 3 getrennten Dateien (index.html,index.css,index.js) aufgeteilt.
### HTML
...
#### Neues Spiel Starten
Die Funktion <b>sendNewGame()</b> fordert ein neues Spiel an und bezieht sich in der Anfrage auf die radio buttons des New-Game-Feldes im HTML-Dokument. Mit der Abfrage "document.querySelector" kann geprüft werden, ob Spieler 1 bzw. Spieler 2 ein menschlicher bzw. ein computergesteuerter Spieler sein soll.

#### HTML-Response verarbeiten
Die Funktion, welche durch das Event "http.onreadystatechange" ausgelöst wird, verarbeitet hauptsächlich den response-String vom Server.
